...
function(){return this.getStackTop().scope};Hub.prototype.getStack=function(){return this._stack};Hub.prototype.getStackTop=function(){return this._stack[this._stack.length-1]};Hub.prototype.captureException=function(exception$jscomp$0,hint){var eventId=this._lastEventId=utils_1.uuid4(),finalHint=hint;if(!hint){try{throw Error("Sentry syntheticException");}catch(exception){hint=exception}finalHint={originalException:exception$jscomp$0,syntheticException:hint}}this._invokeClient("captureException",
exception$jscomp$0,tslib_1.__assign(tslib_1.__assign({},finalHint),{event_id:eventId}));return eventId};Hub.prototype.captureMessage=function(message,level,hint){var eventId=this._lastEventId=utils_1.uuid4(),finalHint=hint;if(!hint){try{throw Error(message);}catch(exception){hint=exception}finalHint={originalException:message,syntheticException:hint}}this._invokeClient("captureMessage",message,level,tslib_1.__assign(tslib_1.__assign({},finalHint),{event_id:eventId}));return eventId};Hub.prototype.captureEvent=
function(event,hint){var eventId=this._lastEventId=utils_1.uuid4();this._invokeClient("captureEvent",event,tslib_1.__assign(tslib_1.__assign({},hint),{event_id:eventId}));return eventId};Hub.prototype.lastEventId=function(){return this._lastEventId};Hub.prototype.addBreadcrumb=function(breadcrumb,hint){var _a=this.getStackTop(),scope=_a.scope;_a=_a.client;if(scope&&_a){_a=_a.getOptions&&_a.getOptions()||{};var _c=_a.beforeBreadcrumb,beforeBreadcrumb=void 0===_c?null:_c;_a=_a.maxBreadcrumbs;_a=void 0===
_a?100:_a;if(!(0>=_a)){_c=utils_1.dateTimestampInSeconds();var mergedBreadcrumb=tslib_1.__assign({timestamp:_c},breadcrumb);breadcrumb=beforeBreadcrumb?utils_1.consoleSandbox(function(){return beforeBreadcrumb(mergedBreadcrumb,hint)}):mergedBreadcrumb;null!==breadcrumb&&scope.addBreadcrumb(breadcrumb,Math.min(_a,100))}}};Hub.prototype.setUser=function(user){var scope=this.getScope();scope&&scope.setUser(user)};Hub.prototype.setTags=function(tags){var scope=this.getScope();scope&&scope.setTags(tags)};
Hub.prototype.setExtras=function(extras){var scope=this.getScope();scope&&scope.setExtras(extras)};Hub.prototype.setTag=function(key,value){var scope=this.getScope();scope&&scope.setTag(key,value)};Hub.prototype.setExtra=function(key,extra){var scope=this.getScope();scope&&scope.setExtra(key,extra)};Hub.prototype.setContext=function(name,context){var scope=this.getScope();scope&&scope.setContext(name,context)};Hub.prototype.configureScope=function(callback){var _a=this.getStackTop(),scope=_a.scope;
_a=_a.client;scope&&_a&&callback(scope)};Hub.prototype.run=function(callback){var oldHub=makeMain(this);try{callback(this)}finally{makeMain(oldHub)}};Hub.prototype.getIntegration=function(integration){var client=this.getClient();if(!client)return null;try{return client.getIntegration(integration)}catch(_oO){return utils_1.logger.warn("Cannot retrieve integration "+integration.id+" from the current Hub"),null}};Hub.prototype.startSpan=function(context){return this._callExtensionMethod("startSpan",
context)};Hub.prototype.startTransaction=function(context,customSamplingContext){return this._callExtensionMethod("startTransaction",context,customSamplingContext)};Hub.prototype.traceHeaders=function(){return this._callExtensionMethod("traceHeaders")};Hub.prototype.startSession=function(context){this.endSession();var _a=this.getStackTop(),scope=_a.scope;_a=(_a=_a.client)&&_a.getOptions()||{};context=new session_1.Session(tslib_1.__assign(tslib_1.__assign({release:_a.release,environment:_a.environment},
scope&&{user:scope.getUser()}),context));scope&&scope.setSession(context);return context};Hub.prototype.endSession=function(){var _a=this.getStackTop(),scope=_a.scope;_a=_a.client;if(scope){var session=scope.getSession();session&&(session.close(),_a&&_a.captureSession&&_a.captureSession(session),scope.setSession())}};Hub.prototype._invokeClient=function(method){for(var args=[],_i=1;_i<arguments.length;_i++)args[_i-1]=arguments[_i];var _b=this.getStackTop();_i=_b.scope;(_b=_b.client)&&_b[method]&&
_b[method].apply(_b,tslib_1.__spread(args,[_i]))};Hub.prototype._callExtensionMethod=function(method){for(var args=[],_i=1;_i<arguments.length;_i++)args[_i-1]=arguments[_i];if((_i=getMainCarrier().__SENTRY__)&&_i.extensions&&"function"===typeof _i.extensions[method])return _i.extensions[method].apply(this,args);utils_1.logger.warn("Extension method "+method+" couldn't be found, doing nothing.")};return Hub}();exports.Hub=Hub;exports.getMainCarrier=getMainCarrier;exports.makeMain=makeMain;exports.getCurrentHub=
function(){var registry=getMainCarrier();registry&&registry.__SENTRY__&&registry.__SENTRY__.hub&&!getHubFromCarrier(registry).isOlderThan(exports.API_VERSION)||setHubOnCarrier(registry,new Hub);if(utils_1.isNodeEnv()){try{var activeDomain=getActiveDomain();if(activeDomain){if(!(activeDomain&&activeDomain.__SENTRY__&&activeDomain.__SENTRY__.hub)||getHubFromCarrier(activeDomain).isOlderThan(exports.API_VERSION)){var registryHubTopStack=getHubFromCarrier(registry).getStackTop();setHubOnCarrier(activeDomain,
