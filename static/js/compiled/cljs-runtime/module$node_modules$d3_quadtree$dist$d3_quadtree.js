...
0;4>i;++i)if(child=node.source[i])child.length?nodes.push({source:child,target:node.target[i]=Array(4)}):node.target[i]=leaf_copy(child);return copy};treeProto.add=function(d){const x=+this._x.call(null,d),y=+this._y.call(null,d);return add(this.cover(x,y),x,y,d)};treeProto.addAll=function(data){var d,i,n=data.length,x,y,xz=Array(n),yz=Array(n),x0=Infinity,y0=Infinity,x1=-Infinity,y1=-Infinity;for(i=0;i<n;++i)isNaN(x=+this._x.call(null,d=data[i]))||isNaN(y=+this._y.call(null,d))||(xz[i]=x,yz[i]=y,
x<x0&&(x0=x),x>x1&&(x1=x),y<y0&&(y0=y),y>y1&&(y1=y));if(x0>x1||y0>y1)return this;this.cover(x0,y0).cover(x1,y1);for(i=0;i<n;++i)add(this,xz[i],yz[i],data[i]);return this};treeProto.cover=function(x,y){if(isNaN(x=+x)||isNaN(y=+y))return this;var x0=this._x0,y0=this._y0,x1=this._x1,y1=this._y1;if(isNaN(x0))x1=(x0=Math.floor(x))+1,y1=(y0=Math.floor(y))+1;else{for(var z=x1-x0||1,node=this._root,parent,i;x0>x||x>=x1||y0>y||y>=y1;)switch(i=(y<y0)<<1|x<x0,parent=Array(4),parent[i]=node,node=parent,z*=2,
i){case 0:x1=x0+z;y1=y0+z;break;case 1:x0=x1-z;y1=y0+z;break;case 2:x1=x0+z;y0=y1-z;break;case 3:x0=x1-z,y0=y1-z}this._root&&this._root.length&&(this._root=node)}this._x0=x0;this._y0=y0;this._x1=x1;this._y1=y1;return this};treeProto.data=function(){var data=[];this.visit(function(node){if(!node.length){do data.push(node.data);while(node=node.next)}});return data};treeProto.extent=function(_){return arguments.length?this.cover(+_[0][0],+_[0][1]).cover(+_[1][0],+_[1][1]):isNaN(this._x0)?void 0:[[this._x0,
...
previous.next=d:delete previous.next,this;if(!parent)return this._root=d,this;d?parent[right]=d:delete parent[right];(node=parent[0]||parent[1]||parent[2]||parent[3])&&node===(parent[3]||parent[2]||parent[1]||parent[0])&&!node.length&&(retainer?retainer[j]=node:this._root=node);return this};treeProto.removeAll=function(data){for(var i=0,n=data.length;i<n;++i)this.remove(data[i]);return this};treeProto.root=function(){return this._root};treeProto.size=function(){var size=0;this.visit(function(node){if(!node.length){do++size;
