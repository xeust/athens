...
arguments[_i];var url=2<args.length?args[2]:void 0;url&&(_i=lastHref,lastHref=url=String(url),triggerHandlers("history",{from:_i,to:url}));return originalHistoryFunction.apply(this,args)}}if(supports_1.supportsHistory()){var oldOnPopState=global.onpopstate;global.onpopstate=function(){for(var args=[],_i=0;_i<arguments.length;_i++)args[_i]=arguments[_i];_i=global.location.href;var from=lastHref;lastHref=_i;triggerHandlers("history",{from,to:_i});if(oldOnPopState)return oldOnPopState.apply(this,args)};
object_1.fill(global.history,"pushState",historyReplacementFunction);object_1.fill(global.history,"replaceState",historyReplacementFunction)}}function instrumentDOM(){"document"in global&&(global.document.addEventListener("click",domEventHandler("click",triggerHandlers.bind(null,"dom")),!1),global.document.addEventListener("keypress",keypressEventHandler(triggerHandlers.bind(null,"dom")),!1),["EventTarget","Node"].forEach(function(target){(target=global[target]&&global[target].prototype)&&target.hasOwnProperty&&
...
function domEventHandler(name,handler,debounce){void 0===debounce&&(debounce=!1);return function(event){keypressTimeout=void 0;event&&lastCapturedEvent!==event&&(lastCapturedEvent=event,debounceTimer&&clearTimeout(debounceTimer),debounce?debounceTimer=setTimeout(function(){handler({event,name})}):handler({event,name}))}}function keypressEventHandler(handler){return function(event){try{var target=event.target}catch(e){return}var tagName=target&&target.tagName;tagName&&("INPUT"===tagName||"TEXTAREA"===
tagName||target.isContentEditable)&&(keypressTimeout||domEventHandler("input",handler)(event),clearTimeout(keypressTimeout),keypressTimeout=setTimeout(function(){keypressTimeout=void 0},debounceDuration))}}function instrumentError(){_oldOnErrorHandler=global.onerror;global.onerror=function(msg,url,line,column,error){triggerHandlers("error",{column,error,line,msg,url});return _oldOnErrorHandler?_oldOnErrorHandler.apply(this,arguments):!1}}function instrumentUnhandledRejection(){_oldOnUnhandledRejectionHandler=
